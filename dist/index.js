...
__export(src_exports, {
  StatefulTabFrameTracker: () => StatefulTabFrameTracker
});
...
var TabFrameTracker = class {
  ...
  async initializeListeners() {
    import_webextension_polyfill.default.tabs.onCreated.addListener((tab) => {
      if (tab.id) {
    ...
    import_webextension_polyfill.default.tabs.onUpdated.addListener(
    ...
    import_webextension_polyfill.default.webNavigation.onCreatedNavigationTarget.addListener(
      (details) => {
    ...
    import_webextension_polyfill.default.webNavigation.onCommitted.addListener(
      (details) => {
        const tabInfo = this.frameMap.get(details.tabId);
    ...
    import_webextension_polyfill.default.webNavigation.onBeforeNavigate.addListener(
      (details) => {
        const tabInfo = this.frameMap.get(details.tabId);
        if (tabInfo && details.frameId) {
    ...
    const existingTabs = await import_webextension_polyfill.default.tabs.query({});
    ...
    import_webextension_polyfill.default.windows.onRemoved.addListener(async (windowId) => {
      const tabs = await import_webextension_polyfill.default.tabs.query({ windowId });
